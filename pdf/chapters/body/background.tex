...
\begin{lstlisting}
console_outputD (console_out_signal);
console_outputD (port_out_signal);
\end{lstlisting}

However, to demonstrate the use of signal combinators in Rizzo, we will modify the output signals to prepend a string to the input values before outputting them to the console. We can use the \texttt{mapD} function from the \texttt{Signal} module to achieve this. \texttt{mapD} takes a function and a later signal as input, and applies the function to each value of the signal.

\begin{lstlisting}
console_outputD (mapD (fun s -> "From console: " ^ s) console_out_signal);
console_outputD (mapD (fun s -> "From port: " ^ s) port_out_signal);
\end{lstlisting}
...
\begin{lstlisting}
port_send_outputD Unix.inet_addr_loopback 9000 console_out_signal;
\end{lstlisting}

To get it to run and block the main thread, we call \lstinline{start_event_loop ();} at the end of the program.

Setting everything together, we get the following complete Rizzo program:
\begin{lstlisting}
open Rizzo.Types
open Rizzo.Signal
open Rizzo.Channel
let () =
    ...
    port_send_outputD Unix.inet_addr_loopback 9000
    console_out_signal;
    start_event_loop ();
\end{lstlisting}

Where an example interaction with the program could look like this:
\begin{verbatim}
Hello Rizzo
