...
The function $ advance : \text{channel } B \rightarrow \text{later } A \rightarrow B \rightarrow A $, implements the \textit{advance semantics}, which define how later values are brought into the current time step if they have ticked. Since the \texttt{later} modality is modelled as a GADT, $advance$ is polymorphically recursive since it recurse over later values whose element types change. Consider the \texttt{App} case in listing \ref{listing:rizzo_advance_code}, where the existential type $c$ introduced from GADT is different from $a$. In order for this case to type check in OCaml, we must use a locally abstract type $a$ to make the polymorphic recursion explicit.
