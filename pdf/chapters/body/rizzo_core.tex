...
It is sufficient with a single mutex for the entire heap, as the semantics of Rizzo are single-threaded. In other words, the only way for data on the heap to be read or changed is during the heap updating process, when delayed computations are advanced. This corresponds exactly to the $inner$ call in listing \ref{rizzo_step}.
