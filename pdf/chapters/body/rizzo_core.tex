...
\todo{Consider the Rizzo program in listing \ref{heap_side_effect}.} The program defines a signal of strings, the tail of which has a side effect, namely printing $[\text{side-effect}]$, and then the program immediately ignores this signal (\todo{should we mention that $\text{@:}$ has a side effect?}).  We would expect the program to do nothing but infinitely hang in the event loop. However, when the program is run and as the first value is produced on the $console\_input$ channel, the program prints $[\text{side-effect}]$. This happens because the garbage collector has yet to run so the ignored signal still sits in the signal heap and is thus updated every time console produces a value.
