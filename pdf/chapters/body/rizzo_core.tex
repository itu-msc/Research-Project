\section{Rizzo Core private methods} \label{section:rizzo_core}
...
start_event_loop ()
\end{lstlisting}
This problem of `ghosts' signals in the heap would disappear in a langauge with reference counting rather than a garbage-collected runtime like that of OCaml. With reference counting, the signal in listing \ref{listing:heap_side_effect} would be immediately discarded after ignore, preventing unexpected artifacts of any side-effects.
...
The function $ advance : \text{channel } B \rightarrow \text{later } A \rightarrow B \rightarrow A $, implements the \textit{advance semantics}, which define how values of type $\text{later }A$ are brought the current time step, if they have ticked. Since the \texttt{later} modality is modelled as a GADT, $advance$ is polymorphically recursive because it may make recursive calls over later values who element types change. Consider the \texttt{App} case in listing \ref{listing:rizzo_advance_code}, where the existential type $c$ introduced from GADT is different from $a$. In this case it is necessary to make the polymorphic recursion of $advance$ explicit, by using a \textit{locally abstract type}, in order for the function to type check.
%Do we cite ocaml GADTS? https://ocaml.org/manual/5.2/gadts-tutorial.html
\begin{lstlisting}[caption={OCaml implementation of the advance semantics of Rizzo including only the pattern match for \texttt{App} (\todo{applicative action}). Existential type $c$ have been made explicit to better show need for polymorphic recursion. }, label={listing:rizzo_advance_code}]
let rec advance 
...
Outputs must also integrate with the event loop. In our implementation this is done by internally creating signals that perform the output operation. For exmaple, in listing \ref{listing:rizzo_io_output} user-provided string signals are mapped to a \texttt{print\_endline} call, which prints to the console. These internally created signals are stored in a mutable list, otherwise the ocaml garbage collector would be allowed to erase them. 
...
This design of outputs integrates nicely with the event loop, since it is handled automatically as part of updating the heap. However, it comes with a significant trade-off. It introduces side effects into the heap and forces the implementation of the language to account for side effects. 
