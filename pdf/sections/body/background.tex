...
\begin{itemize}
  ...
  \item $\texttt{filter}: (A \to \text{bool}) \to A \text{ signal} \to (A \text{ signal}) \text{ later}$: keeps only values that satisfy a predicate. Noteably filter returns later value because the head of the signal may not satisfy the predicate. 
...
\paragraph{A note on later values and the L suffix.} Some combinators are also provided in variants with an L suffix (for example, $map$ and $mapL$). These variants are intended to make it convenient to work with with delayed signals, that is, values of type $(A \text{ signal}) \text{ later}$, which are produced by combinators such as \texttt{filter} and \texttt{mkSig}.
