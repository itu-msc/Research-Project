...
This section gives a practical overview of how Rizzo can be used to build functional reactive programs. We explain (1) how to create input signals, (2) how to transform them using signal combinators, and (3) how to connect signals to outputs. We then combine these pieces in a small example program, and finish with a short reference of the most important functions and types.

\subsection{Inputs}
Input sources in Rizzo are called \emph{channels}. In principle, a channel can be implemented for many kinds of input sources, but the current library only provides channels for console input, a clock, and network ports.
...
\paragraph{A note on \texttt{later}.}
Many constructors return a value of type \texttt{'a signal later}. Intuitively, this means the signal is available one step later (e.g., after the runtime has observed an event from the channel). Most combinators and output functions therefore come in two variants: one for plain signals and one for later signals, indicated by an \texttt{L} suffix (e.g., \texttt{map} vs.\ \texttt{mapL}).

\subsection{Signal combinators}
Signals are the main abstraction in Rizzo, and the library provides a number of combinators for transforming them. Some of the most common ones are \texttt{map}, \texttt{filter}, and \texttt{sample}. As mentioned above, most combinators come in a plain variant and an \texttt{L}-variant that works on values of type \texttt{signal later}.
