...
When input is produced on a some channel \(k\), the time \emph{ticks} and the program enters a new time step. The step semantics update the heap so that all signals reflect this new step.

To perform the update, our runtime implements a single update function
\[
  \text{step} : B \text{ channel} \rightarrow B \rightarrow unit,
\]
which takes the channel that produced the event and the event payload. The update proceeds in two phases:

\paragraph{(1) Initialize the step.}
We start by running a garbage-collection cycle to reclaim signals that have become unreachable since the last step.
We then reset the cursor to the head of the heap, which conceptually is the same as moving everything to the \textit{earlier} heap.
...
\begin{enumerate}
  ...
  \item Otherwise, let the node contain signal data \((v\langle U \rangle w)\). We test whether the tail \(w\) \emph{ticks} with respect to the incoming channel event by calling
    \[
      \text{ticked} : B \text{ channel} \rightarrow A \text{ later} \rightarrow \text{bool}.
    \]
...
Overwriting existing heap nodes is efficient, but it interacts subtly with weak references and garbage collection. In particular, advancing a recursive tail (e.g. a \texttt{map}-like combinator) typically allocates a new signal \(s'\) and then copies \(s'\)'s tail into the existing signal \(s\) by mutation. If \(s'\) remains present in the heap as a ghost (because no GC cycle has yet cleared its weak reference), then on the next tick both \(s\) and \(s'\) may be visited and advanced, duplicating work and potentially duplicating side effects. Repeating this can lead to rapid heap growth.

We mitigate this in our implementation by triggering a major garbage-collection cycle at the \emph{start} of each \textit{step}. This encourages the runtime to clear weak references for unreachable heap entries, so that lingering ghost signals are removed before we traverse and update the heap, reducing both unnecessary work and the risk of accumulating duplicates.

A better solution, if it were available to us, would be reference counting. Since the heap stores only weak references to duplicates, reference counting would reclaim them immediately once they become unreachable, eliminating ghost entries and the resulting duplication without requiring explicit GC cycles.
...
\[
  \text{App} : (c \rightarrow a) \text{ delayOnce}  \rightarrow a \text{ later} \rightarrow c \text{ later}
\]
We must first advance $x : (c \text{ later})$ in order to pass its value to $f : \text{delayOnce }(c \rightarrow a)$. 
At the outer call \texttt{advance} has type $b \text{ channel}  \rightarrow \; a \text{ later} \rightarrow b \rightarrow a$ but inside the branch of \texttt{App} we need to call a type $b \text{ channel} \rightarrow \; c \text{ later} \rightarrow b \rightarrow c$. This change in type parameter across recursive calls is exactly what requires polymorphic recursion.
