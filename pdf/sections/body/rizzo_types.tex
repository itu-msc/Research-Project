...
type 'a signal_simple = Sig of ('a * 'a signal_simple later) ref
\end{lstlisting}

Implimentation wise the signal type not as simple as we have described it so far though.
We also need to keep track of the signal's unique id and whether it has been updated in the current time step. 
This is necessary for the Rizzo runtime to be able to manage the signals and ensure that they are updated correctly.
Given the old part of the signal also must not exist, then we can make the head and tail mutable, so they can be updated in place. This improves performance and reduces memory usage.
% Signals new redundant signals might still be created during signal combinator operations, but the old signals will be garbage collected by OCaml's garbage collector when they are no longer needed. So over time the memory usage will not grow by the tails of the signals.

The internal type signature of signals is therefore defined as:
