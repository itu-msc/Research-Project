...
Functional reactive programming (FRP)~\cite{ElliottHudak97:Fran} is a high-level paradigm for building reactive programs. It lets programmers use familiar functional programming techniques to implement and reason about time-flow and events, and to describe time-varying values and events declaratively rather than with low-level callbacks or mutable state.
...
Naively representing signals as infinite streams makes it easy to write programs that violate either \emph{causality} or \emph{productivity}: each output depends
only on the current and past inputs, and each input eventually produces an
output. Futhermore, such representations are prone to \emph{space leaks}, where
old signal values can be retained in memory indefinitely~\cite{bahr_asynchronous_2023}.

\subsection{Modal Types}
To avoid space leaks (and to guarantee other operational properties such as causality and productivity), previous works use \textit{modal types}~\cite{neel_krishnaswami_2013,gebser_asynchronous_2023}.
...
\] 
Here, only the current value is available now, while the next value is available later.
...
\] 
The type $\square (a \rightarrow b)$ is crucial here, as it enforces that the function is time-independent. This disallows the function from capturing references to old values which would otherwise cause space leaks.
...
However, this elegant solution comes at the cost of added complexity for both the type checker and users. Whenever a function like $map$ is used, the type checker must ensure, through typing rules, that the provided function only uses other stable variables\cite{bahr_asynchronous_2023}. Additionally, programmers must perform a dance with the compiler to make types fit. For example, consider $mult2 : \text{Sig }a \rightarrow \text{Sig }a$, a function that takes a signal and produces a new signal where each value is multiplied by two:
\[
  mult2 \; xs = map \; (box \;(\lambda x.\ x * 2)) \; xs
\]
Note in particular the use of $box$ (the introduction form for $\square$), which is required to satisfy the stable modality of $map$.
...
The mutable references in the semantics of signals is what allows Rizzo to forego the $\square$ (stable) modality. Since signals are updated in place, dereferencing a signal always yields its current value, making earlier heads inaccessible via the signal itself. Therefore, we can give $map$ the familiar type $map : (a \rightarrow b) \rightarrow \text{Sig }a \rightarrow \text{Sig }b$ without requiring a stable modality, allowing us to redefine $mult2$ as:
\[
  mult2 \; xs = map \; (\lambda x. x * 2) \; xs
\]
Notice that the use of $box$ is gone and $map$ has taken its familiar form.

Furthermore, removing the stable modality also allows a larger space of well-typed programs. Consider the function $sample : \text{Sig }A \rightarrow \text{Sig }B \rightarrow \text{Sig }(A \times B)$ defined as:
\[
  sample \; xs \; ys = map \; (\lambda x. \langle x , head \; ys \rangle ) \; xs
\]
The $sample$ function takes two signals $xs$ and $ys$ and creates a new signal that samples the head of $ys$ whenever $xs$ produces a value $x$ and then produces a pair $(x, head \; ys)$. This program would fail to type-check with the previous definition of $map$, since the lambda is not stable as it references $ys$. The reference to $ys$ would cause a space leak as old values of $ys$ must be kept in memory. However, with Rizzo's semantics, it is safe for the lambda to reference $ys$ directly, because $ys$ is updated in place.
...
Rizzo is also asynchronous, meaning that different signals are not necessarily updated at the same time. Rizzo allows each input source to produce events at its own pace, while the runtime ensures that only one event is processed at a time. This makes programs like $sample$ useful, as $xs$ can trigger reads of the current head of $ys$ without requiring $ys$ to update at the same moment. An example of using $sample$ to read data from one signal based on an event occurring on another signal is demonstrated in Section~\ref{section:rizzo_example}.
...
\begin{itemize}
  \item We design and implement a small Rizzo-style FRP library in OCaml,
    closely following the operational semantics described in \citetitle{rizzo_modal_2025}\cite{rizzo_modal_2025}.
    Section~\ref{using_rizzo} illustrates how to use this library to build
    simple reactive programs.
  \item We discuss practical challenges of embedding Rizzo in OCaml, in particular the interaction between the heap, weak references, garbage collection, and side effects, and we describe the mitigation used by our runtime.
\end{itemize}
